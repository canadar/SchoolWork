...
void MeshGLWidget::initializeGL()
{
    ...
    initShader(":/Animatrix/Material.vert", ":/Animatrix/Material.frag");
}
...
void MeshGLWidget::drawContents()
{
    ...
    mMatrixContext.pushModelMatrix();
        ...
        if(mDrawNormals)
        {
            ...
            mObject->drawNormals(mMatrixContext, mShader);
        }
...
void MeshGLWidget::drawAxies()
{
    ...
    glBegin(GL_LINES);
        glColor4fv(sRed);
        glVertex3f(0.0, 0.0, 0.0); glVertex3f(10, 0.0, 0.0);

        glColor4fv(sGreen);
        glVertex3f(0.0, 0.0, 0.0); glVertex3f(0.0, 10, 0.0);

        glColor4fv(sBlue);
        glVertex3f(0.0, 0.0, 0.0); glVertex3f(0.0, 0.0, 10);
    glEnd();
...
void MeshGLWidget::changeCurrentFrame(int pFrame)
{
    if(mObject != NULL) mObject->changeFrame(pFrame);
}
