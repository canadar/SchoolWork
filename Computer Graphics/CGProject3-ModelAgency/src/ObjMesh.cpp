...
uint32_t ObjMesh::getNamedGroupCount() { return names.size(); }
...
GLvoid ObjMesh::addRawVertex(const float X, const float Y, const float Z)
{
...
GLvoid ObjMesh::addRawNormal(const float i, const float j, const float k)
{
...
bool ObjMesh::parseRawData(bool pAllowEmpty)
{
    ...
    for(int i=0; i<(int)group.size(); i++)
        lNumberOfTris += group[i]->triangleV.size()/3;
...
bool ObjMesh::computePerFaceNormals(bool overwrite)
{
    ...
    for(uint32_t j=0; j<group.size(); j++)
    {
        ...
        for(uint32_t i=0; i<curGroup->triangleV.size(); i+=3)
        {
            ...
            uint32_t normalIndex = (rawNorms.size()-3)/3;
...
bool ObjMesh::writeOBJFile(string filename)
{
    ...
    for(uint32_t i=0; i<group.size(); i++)
        if(group[i] != NULL) totalFaces += group[i]->triangleV.size()/3;
